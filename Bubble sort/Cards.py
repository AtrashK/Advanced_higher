...
def sort_cards(cards, total_swaps):
    for outer in range (len(cards)-1, 0, -1):
        for inner in range(outer):
            if (cards[inner].value > cards[inner+1].value) or ((cards[inner].value == cards[inner+1].value) and (suit_value(cards[inner]) > suit_value(cards[inner+1]))):
                cards[inner], cards[inner+1] = cards[inner+1], cards[inner]
                total_swaps += 1
